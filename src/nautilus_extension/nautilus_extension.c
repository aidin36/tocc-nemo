 ...
 */
static void on_tocc_import_activate(NautilusMenuItem* item,
                                    gpointer user_data)
{
  GList* files;
  GList* file_iterator;
  ...
  {
    NautilusFileInfo* file_info = NAUTILUS_FILE_INFO(file_iterator->data);

    GFile* gfile = (GFile*)nautilus_file_info_get_location(file_info);
 ...
 * of Nautilus.
 */
GList* tocc_extension_get_file_items(NautilusMenuProvider* provider,
                                     GtkWidget* window,
                                     GList* files)
{
  ...
  {
    if (nautilus_file_info_is_directory(files_iterator->data))
    {
  ...
  item = nautilus_menu_item_new("ToccExtension::import",
                            "Import to Tocc",
...
GList* tocc_extension_get_background_items(NautilusMenuProvider* provider,
                                           GtkWidget* window,
 ...
 */
static void tocc_extension_menu_provider_iface_init(NautilusMenuProviderIface* iface)
{
 ...
 * It tells Nautilus what types this extension provides.
 */
...
// Following two methods are un-unused. But they should be provided for
